...
export function createTapUI(
    { tapValue, averageValue, tapHistory },
    { tapEngine, getCurrentLanguage, formatBpm, getTranslations },
) {
    /* =====================================================
       TAP DISPLAY
       ===================================================== */
    ...
    function showTap() {
        tapValue.textContent = getTranslations(getCurrentLanguage()).tap;
    }
    ...
    function showAverageBpm(bpm) {
        if (Number.isFinite(bpm)) {
            averageValue.textContent = formatBpm(bpm);
        } else {
    ...
    function updateLanguage(language) {
        ...
        const currentValue = tapValue.textContent.trim();

        const isTapState = currentValue === "TAP" || currentValue === "ТАП";

        if (isTapState) {
            tapValue.textContent = text.tap;
        }

        showAverageBpm(tapEngine.getAverageBpm());
    }
    ...
    function renderTapHistory(timestamps) {
        ...
        const historyWidth = tapHistory.clientWidth;

        const usableWidth = Math.max(
            0,
            historyWidth - leftPadding - rightPadding,
        );
        ...
        if (timestamps.length === 1 || timeRange <= 0) {
            createTapPoint(50);
        ...
        timestamps.forEach((timestamp) => {
            ...
            const x = leftPadding + normalized * usableWidth;

            const percent = historyWidth > 0 ? (x / historyWidth) * 100 : 50;

            createTapPoint(percent);
        });

        renderTapConnectors(timestamps, leftPadding, usableWidth, historyWidth);
    }
    ...
    function createTapPoint(leftPercent) {
        ...
        point.className = "tap-point";

        point.style.left = `${leftPercent}%`;

        point.style.top = "50%";

        tapHistory.appendChild(point);
    }
    ...
    function renderTapConnectors(
        ...
        if (timeRange <= 0 || historyWidth <= 0) {
            return;
        }

        for (let index = 0; index < timestamps.length - 1; index += 1) {
            const startNormalized = (timestamps[index] - firstTime) / timeRange;

            const endNormalized =
                (timestamps[index + 1] - firstTime) / timeRange;

            const startX = leftPadding + startNormalized * usableWidth;

            const endX = leftPadding + endNormalized * usableWidth;

            const connector = document.createElement("div");

            connector.className = "tap-connector";
            ...
            tapHistory.appendChild(connector);
        }
    }
